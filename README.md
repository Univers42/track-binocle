...
The Gmail bridge app is Docker-managed from the repository root:
...
The UI runs at `http://localhost:3002` and the bridge runs at `http://localhost:4100`. Google OAuth credentials belong in `apps/mail/.env.local` or the BaaS Vault secret configured by the mail app.
