...
After `make docker-rm-all`, every image layer must be downloaded again. `make all` runs `make docker-prefetch-images` before Compose builds; that target pulls public runtime images plus the `docker/dockerfile:1` BuildKit frontend, tries public mirrors where available, retries each pull, and bounds every pull with `DOCKER_PULL_TIMEOUT` seconds plus `DOCKER_PULL_KILL_AFTER` seconds for hard termination. The later Compose `up` calls use `--pull never`, so any registry or networking problem stays inside the bounded prefetch step. The Makefile exports `BUILDX_BUILDER=default`, which keeps Compose builds on Docker's built-in builder even if the user's shell has selected a docker-container Buildx builder. If all mirror and direct pulls fail, the pipeline exits with a Docker networking or registry error instead of waiting indefinitely. The default pipeline still fails before these pulls when credentials are missing.