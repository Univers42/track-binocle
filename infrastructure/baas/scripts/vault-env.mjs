...
const managedFiles = [
  ...
  },
  {
    ...
    envPath: 'infrastructure/baas/.env.local',
    examplePath: 'infrastructure/baas/.env.example',
  ...
  },
  {
    ...
    envPath: 'infrastructure/baas/mini-baas-infra/.env',
    examplePath: 'infrastructure/baas/mini-baas-infra/.env.example',
...
function renderEnv(config, values, { example = false } = {}) {
  ...
  const lines = [
    '# -----------------------------------------------------------------------------',
    `# ${config.title}`,
    '# Managed by infrastructure/baas/scripts/vault-env.mjs.',
    example
...
if (command === 'format') {
  ...
  console.log('Usage: node infrastructure/baas/scripts/vault-env.mjs <format|seed|fetch|backup|roundtrip|verify-approles>');
}